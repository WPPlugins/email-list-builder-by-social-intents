...
Description: Add a customizable and targeted email subscription overlay to any page.  Integrates with MailChimp and Constant Contact as well as CSV Exports (more coming soon).  To get started: 1) Click the "Activate" link to the left of this description, 2) Go to your Email List Builder configuration page, and register for a new account.
Version: 1.3.42
...
define('SIELB_SMALL_LOGO',plugin_dir_url( __FILE__ ).'si-small.png');
define('SIELB_DASHBOARD_URL', "https://www.socialintents.com/dashboard.do");
...
function si_dashboard() {
      ...
      echo '<div id="dashboarddiv"><iframe id="dashboardiframe" src="'.SIELB_DASHBOARD_URL.'" height=500 width=98% scrolling="yes"></iframe></div>      
	<a href="'.SIELB_DASHBOARD_URL.'" target="_newWindow" onClick="javascript:document.getElementById(\'dashboarddiv\').innerHTML=\'\'; ">Open Email List Builder by Social Intents in a new window</a>.
      ';
}
...
function elb_insert() {
    ...
    global $current_user;
    if(strlen(get_option('elb_widgetID')) == 32) {
	wp_get_current_user();
	...
	echo("(function() {function socialintents(){\n");
        ...
        echo("    var s = document.createElement('script');s.type = 'text/javascript';s.async = true;s.src = siJsHost+'www.socialintents.com/api/email/socialintents.js#".get_option('elb_widgetID')."';\n");
    ...
    <div class="metabox-holder meta-box-sortables ui-sortable pointer">
        <div class="postbox" style="float:left;width:30em;margin-right:10px">
			...
			<?php echo '<img src="'.plugins_url( 'socialintents.png' , __FILE__ ).'" height="150" "/> ';?></a></p>
                    ...
                    <p><label for="elb_widgetID"><?php printf(__('Register for your account to the right to activate your plugin right now.  If you don\'t have your key but have already signed up, you can <a href=\'http://www.socialintents.com\' target=\'_blank\'>login here</a> to grab your key under your Apps --> Email List Builder --> Edit Settings --> Widget Key<br>', $elb_domain), '<strong><a href="http://www.socialintents.com/" title="', '">', '</a></strong>') ?></label><br />
...
$.ajax({
   type: 'GET',
    ...
    add_submenu_page('options-general.php', __('List Builder Settings', $elb_domain), __('List Builder Settings', $elb_domain), 'manage_options', 'email-list-builder-by-socialintents', 'elb_settings_page');
     add_menu_page('Account Configuration', 'List Builder', 'administrator', 'si_dashboard', 'si_dashboard', SIELB_SMALL_LOGO);
